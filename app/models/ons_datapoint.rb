...
#  default_scope :include => {:ons_dataset_topic => :ons_dataset_family}
  ...
  named_scope :with_topic_uids, lambda { |ons_uids| { :conditions => ["ons_datapoints.ons_dataset_topic_id = ons_dataset_topics.id AND ons_dataset_topics.ons_uid in (?)", ons_uids], 
                                                      :joins => "INNER JOIN ons_dataset_topics", 
                                                      :group => 'ons_datapoints.id'} }

  named_scope :with_topic_grouping, { :conditions => "ons_datapoints.ons_dataset_topic_id = ons_dataset_topics.id AND ons_dataset_topics.dataset_topic_grouping_id IS NOT NULL", 
                                      :joins => "INNER JOIN ons_dataset_topics", 
                                      :group => 'ons_datapoints.id'}

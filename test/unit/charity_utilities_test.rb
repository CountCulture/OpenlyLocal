  ...
  context "A Client instance" do
    ...
    context "when finding new charities" do
      setup do
        ...
        HTTPClient.any_instance.stubs(:post).returns(stub(:header => {'Location' => ['foo.com']}))
      end
    ...
    context "when getting charity details" do
      ...
      context "and when charity is big one" do
        should 'extract assets and people info from front page' do
          ...
          assert_equal '1911', info[:employees]
          assert_nil info[:volunteers]
          asset_info = info[:financial_breakdown][:assets]
          assert_equal '33849000', asset_info[:own_use_assets]
          assert_equal '48647000', asset_info[:long_term_investments]
          assert_equal '10486000', asset_info[:other_assets]
          assert_equal '-25639000', asset_info[:total_liabilities]
        end
        ...
        should 'extract income info from front page as part of financial_breakdown' do
          ...
          assert_equal "114423000", income[:voluntary]
          assert_equal "7793000", income[:trading]
          assert_equal "2372000", income[:investment]
          assert_equal "11115000", income[:charitable]
          assert_equal "0", income[:other]
          assert_equal "199000", income[:investment_gains]
        end

        should 'extract spending info from front page as part of financial_breakdown' do
          breakdown = @client.get_details[:financial_breakdown]
          assert income = breakdown[:spending]
          assert_equal "24311000", income[:generating_voluntary_income]
          assert_equal "868000", income[:governance]
          assert_equal "4671000", income[:trading]
          assert_equal "105000", income[:investment_management]
          assert_equal "106861000", income[:charitable_activities]
          assert_equal "1987000", income[:other]
        end

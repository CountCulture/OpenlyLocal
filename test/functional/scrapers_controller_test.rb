...
class ScrapersControllerTest < ActionController::TestCase
  ...
  context "POST to :scrape" do
    ...
    should "add to delayed job queue" do
      ...
      # Delayed::Job.expects(:enqueue).with(instance_of())
  ...
  context "on GET to :new" do
    ...
    context "when council has portal_system but explicit portal system specified" do
      setup do
        @general_portal_system = Factory(:portal_system, :name => "Some Portal for Councils")
        @general_portal_system.parsers << @gen_parser = Factory(:another_parser) # add a parser to portal_system...
        @council.update_attribute(:portal_system_id, @general_portal_system.id)# .. and associate portal_system to council
        @portal_system = Factory(:portal_system, :name => "Specific Portal System")
        @portal_system.parsers << @parser = Factory(:another_parser) # add a parser to portal_system...
        @parser = Factory(:parser, :portal_system => @portal_system)
        stub_authentication
        get :new, :type  => "ItemScraper", :result_model => @parser.result_model, :council_id => @council.id, :portal_system_id => @portal_system.id
